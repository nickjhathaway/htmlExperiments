mipOverlapWorker = function() {
  ...
  function computeNodeColorByMipNum() {
    ...
    var color = d3.scaleOrdinal(d3.schemeCategory20).domain([minMipNum, maxMipNum]);
  ...
  function computeNodeColorFrac() {
    ...
    var color = d3.scaleOrdinal(d3.schemeCategory20).domain([minNum, maxNum]);
    nodes.forEach(function(node) {
      if(!node.hasOwnProperty("color")){
...
function MipOverlapper(addTo, graph, width, height, margin){
	...
	this.margin = margin || {top: 20, right: 20, bottom: 6, left: 20};
